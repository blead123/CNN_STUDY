...
def softmax_regression():##로지스틱 회귀를 여러번 사용 c,a,b를 판단하는데 사용 -->클래스의 갯수만큼 내부적 으로 통합
    ## [공부시간 , 출석일수]
    x = [[1, 2], [2, 3], [4, 5], [5, 4], [8, 9], [9, 8]]
    ## 0->탈락 1->통과
    ## 로지스틱 이므로 0~1 사이 값 소프트맥스는 원-핫벡터로 표현
    #     a b c
    y = [[0,0,1],
         [0,0,1],
         [0,1,0],
    ...
    model = keras.models.Sequential()
    ###로지스틱이랑 멀티플이랑 다름 -->cnn에선 멀티플 안씀
    model.add(keras.layers.Dense(3, keras.activations.softmax))  ##모든 출력의 결과가 0~1사이 w=x+b
    model.compile(optimizer=keras.optimizers.SGD(0.1),  ##옵티마이저가 값로스를 줄이면서 최적화에 가까워짐
                  loss=keras.losses.categorical_crossentropy, metrics='acc')  ##로그 방정식을 이용해 손실을 계산
